...
$provide.decorator('$animate',['$delegate','$injector','$sniffer','$rootElement','$$asyncCallback','$rootScope','$document',function($delegate,$injector,$sniffer,$rootElement,$$asyncCallback,$rootScope,$document){var globalAnimationCounter=0;$rootElement.data(NG_ANIMATE_STATE,rootAnimateState);$rootScope.$$postDigest(function(){$rootScope.$$postDigest(function(){rootAnimateState.running=false;});});var classNameFilter=$animateProvider.classNameFilter();var isAnimatableClassName=!classNameFilter?function(){return true;}:function(className){return classNameFilter.test(className);};function lookup(name){if(name){var matches=[],flagMap={},classes=name.substr(1).split('.');if($sniffer.transitions||$sniffer.animations){matches.push($injector.get(selectors['']));}
...
return matches;}}
function animationRunner(element,animationEvent,className){var node=element[0];if(!node){return;}
var isSetClassOperation=animationEvent=='setClass';var isClassBased=isSetClassOperation||animationEvent=='addClass'||animationEvent=='removeClass';var classNameAdd,classNameRemove;if(angular.isArray(className)){classNameAdd=className[0];classNameRemove=className[1];className=classNameAdd+' '+classNameRemove;}
var currentClassName=element.attr('class');var classes=currentClassName+' '+className;if(!isAnimatableClassName(classes)){return;}
...
if(runner.isClassBased&&!runner.isSetClassOperation&&!skipAnimation){skipAnimation=(animationEvent=='addClass')==element.hasClass(className);}
if(skipAnimation){fireDOMOperation();fireBeforeCallbackAsync();fireAfterCallbackAsync();fireDoneCallbackAsync();return;}
if(animationEvent=='leave'){element.one('$destroy',function(e){var element=angular.element(this);var state=element.data(NG_ANIMATE_STATE);if(state){var activeLeaveAnimation=state.active['ng-leave'];if(activeLeaveAnimation){activeLeaveAnimation.cancel();cleanup(element,'ng-leave');}}});}
element.addClass(NG_ANIMATE_CLASS_NAME);var localAnimationCount=globalAnimationCounter++;totalActiveAnimations++;runningAnimations[className]=runner;element.data(NG_ANIMATE_STATE,{last:runner,active:runningAnimations,index:localAnimationCount,totalActive:totalActiveAnimations});fireBeforeCallbackAsync();runner.before(function(cancelled){var data=element.data(NG_ANIMATE_STATE);cancelled=cancelled||!data||!data.active[className]||(runner.isClassBased&&data.active[className].event!=animationEvent);fireDOMOperation();if(cancelled===true){closeAnimation();}else{fireAfterCallbackAsync();runner.after(closeAnimation);}});function fireDOMCallback(animationPhase){var eventName='$animate:'+animationPhase;if(elementEvents&&elementEvents[eventName]&&elementEvents[eventName].length>0){$$asyncCallback(function(){element.triggerHandler(eventName,{event:animationEvent,className:className});});}}
function fireBeforeCallbackAsync(){fireDOMCallback('before');}
function fireAfterCallbackAsync(){fireDOMCallback('after');}
function fireDoneCallbackAsync(){fireDOMCallback('close');if(doneCallback){$$asyncCallback(function(){doneCallback();});}}
...
element.data(NG_ANIMATE_CSS_DATA_KEY,{running:formerData.running||0,itemIndex:itemIndex,stagger:stagger,timings:timings,closeAnimationFn:noop});var isCurrentlyAnimating=formerData.running>0||animationEvent=='setClass';if(transitionDuration>0){blockTransitions(element,className,isCurrentlyAnimating);}
if(animationDuration>0&&stagger.animationDelay>0&&stagger.animationDuration===0){blockKeyframeAnimations(element);}
return true;}
...
function animateRun(animationEvent,element,className,activeAnimationComplete){var node=extractElementNode(element);var elementData=element.data(NG_ANIMATE_CSS_DATA_KEY);if(node.getAttribute('class').indexOf(className)==-1||!elementData){activeAnimationComplete();return;}
...
try{webStorage.removeItem(deriveQualifiedKey(key));if(notify.removeItem){$rootScope.$broadcast('LocalStorageModule.notification.removeitem',{key:key,storageType:this.storageType});}}catch(e){$rootScope.$broadcast('LocalStorageModule.notification.error',e.message);return removeFromCookies(key);}
...
if(thisCookie.indexOf(deriveQualifiedKey(key)+'=')===0){return decodeURIComponent(thisCookie.substring(prefix.length+key.length+1,thisCookie.length));}}
...
var key=thisCookie.substring(prefixLength,thisCookie.indexOf('='));removeFromCookies(key);}};var getStorageType=function(){return storageType;};var bindToScope=function(scope,key,def){var value=getFromLocalStorage(key);if(value===null&&angular.isDefined(def)){value=def;}else if(angular.isObject(value)&&angular.isObject(def)){value=angular.extend(def,value);}
scope[key]=value;scope.$watchCollection(key,function(newVal){addToLocalStorage(key,newVal);});};return{isSupported:browserSupportsLocalStorage,getStorageType:getStorageType,set:addToLocalStorage,add:addToLocalStorage,get:getFromLocalStorage,keys:getKeysForLocalStorage,remove:removeFromLocalStorage,clearAll:clearAllFromLocalStorage,bind:bindToScope,deriveKey:deriveQualifiedKey,cookie:{set:addToCookies,add:addToCookies,get:getFromCookies,remove:removeFromCookies,clearAll:clearAllFromCookies}};}];});}).call(this);(function(angular){'use strict';angular.module('twitStream',['ngRoute','ngSanitize','ngAnimate','LocalStorageModule']).config(Config);function Config($routeProvider){$routeProvider.when('/',{templateUrl:'/partial/stream.html',controller:'StreamController',controllerAs:'Stream'}).when('/storage',{templateUrl:'/partial/favorites.html',controller:'FavoritesController',controllerAs:'Fav'}).otherwise({redirectTo:'/'});}})(angular);(function(angular){'use strict';angular.module('twitStreamMock',[]);})(angular);(function(angular){'use strict';angular.module('twitStream').controller('FavoritesController',FavoritesController);FavoritesController.$injector=['$scope','$timeout','$location','Storage'];function FavoritesController($scope,$timeout,$location,Storage){var vm=this;$scope.$location=$location;$scope.controller='FavoritesController';vm.removedFromFavorites=false;vm.tweetIndex=null;vm.favoritesList=false;vm.q='';vm.getAllFromFavorites=function(){var storageData=Storage.getFromFavorites();vm.favoritesList=(angular.equals([],storageData))?false:storageData;};vm.removeFromFavorites=function(tweet){vm.tweetIndex=vm.favoritesList.indexOf(tweet);Storage.remove(tweet.idStr);};vm.removeAllFromFavorites=function(){if(!angular.equals([],vm.favoritesList)&&vm.favoritesList){Storage.removeAll();}};if(typeof $scope.$$listeners['favorites.removed']==='undefined'){$scope.$on('favorites.removed',function(event,response){if(response.status.type==='ok'){vm.favoritesList.splice(vm.tweetIndex,1);}
if(angular.equals([],vm.favoritesList)){vm.favoritesList=false;}
vm.removedFromFavorites=true;$scope.favoritesResponse=response.status;$timeout(function(){vm.removedFromFavorites=false;},1500);});}
if(typeof $scope.$$listeners['favorites.removedAll']==='undefined'){$scope.$on('favorites.removedAll',function(event,response){if(response.status.type==='ok'){vm.favoritesList=false;}
vm.removedFromFavorites=true;$scope.favoritesResponse=response.status;$timeout(function(){vm.removedFromFavorites=false;},1500);});}}})(angular);(function(angular){'use strict';angular.module('twitStream').controller('StreamController',StreamController);StreamController.$inject=['$scope','$timeout','$location','$interval','Socket','Tweet','StreamStatus','Storage'];function StreamController($scope,$timeout,$location,$interval,Socket,Tweet,StreamStatus,Storage){var vm=this;var stop;$scope.$location=$location;$scope.controller='StreamController';$scope.addedToFavorites=false;$scope.alert={active:false,activeFor:0};vm.socketInited=StreamStatus.get('socketInited')||false;vm.loading=StreamStatus.get('loading')||false;vm.showHint=StreamStatus.get('showHint')||false;vm.hasResults=StreamStatus.get('hasResults')||false;vm.results=StreamStatus.get('results')||[];vm.socketId=StreamStatus.get('socketId')||undefined;vm.search=function(){var streamOn=StreamStatus.get('connected')||false;vm.channels=vm.channels||StreamStatus.get('channels');if(vm.channels){vm.loading=true;if(!vm.socketInited||streamOn){if(!streamOn){Socket.connect();}}
var channels=vm.channels.split(',');Socket.emit('addChanel',channels);StreamStatus.set('channels',vm.channels);StreamStatus.set('showHint',false);StreamStatus.set('hasResults',true);StreamStatus.set('results',vm.results);StreamStatus.set('connected',true);}};vm.toggleHint=function(){vm.showHint=!vm.showHint;};vm.addToFavorites=function(tweet){tweet.addedTimestamp=Date.now();Storage.addToFavorites(vm.socketId,tweet);};vm.isFavorited=function(tweetId){var added=Storage.isStored(tweetId);return added;};vm.startAlert=function(waitSec){if(angular.isDefined(stop)){return;}
$scope.alert={active:true,activeFor:waitSec};stop=$interval(function(){if(waitSec>=1){$scope.alert.activeFor=waitSec--;}
else{$scope.alert={active:false,activeFor:1};}},1000);};vm.stopAlert=function(){if(angular.isDefined(stop)){$interval.cancel(stop);stop=undefined;}
$scope.alert={active:false};};if(typeof $scope.$$listeners['favorites.added']==='undefined'){$scope.$on('favorites.added',function(event,response){$scope.addedToFavorites=true;$scope.favoritesResponse=response.status;$timeout(function(){$scope.addedToFavorites=false;},1500);});}
if(typeof $scope.$$listeners['socket.updates']==='undefined'){$scope.$on('socket.updates',function(event,response){$scope.$apply(function(){vm.stopAlert();vm.hasResults=true;vm.loading=false;vm.results.unshift(Tweet.parse(response.data));StreamStatus.set('loading',false);});});}
if(typeof $scope.$$listeners['socket.connected']==='undefined'){$scope.$on('socket.connected',function(event,response){vm.socketInited=true;vm.socketId=response.socketId;StreamStatus.set('socketInited',true);StreamStatus.set('socketId',response.socketId);});}
if(typeof $scope.$$listeners['socket.disconnected']==='undefined'){$scope.$on('socket.disconnected',function(){vm.socketInited=false;StreamStatus.set('socketInited',false);});}
if(typeof $scope.$$listeners['twitter.connected']==='undefined'){$scope.$on('twitter.connected',function(){vm.stopAlert();});}
if(typeof $scope.$$listeners['twitter.reconnecting']==='undefined'){$scope.$on('twitter.reconnecting',function(event,response){vm.startAlert(response.waitSec);});}
$scope.$on('$destroy',function(){vm.stopAlert();});}})(angular);(function(angular){'use strict';angular.module('twitStream').directive('tweet',Tweet);function Tweet(){var directive={restrict:'E',scope:true,replace:true,templateUrl:'/partial/tweet_template.html'};return directive;}})(angular);(function(angular){'use strict';angular.module('twitStream').factory('Socket',Socket);function Socket($rootScope){var _socket=null;var service={emit:emit,connect:connect};return service;function emit(eventName,data){var emitData={};if(typeof data!=='undefined'){emitData={params:data};}
_socket.emit(eventName,emitData);}
function connect(){_socket=io.connect('/');listen();}
function listen(){_socket.on('connect',function(){$rootScope.$broadcast('socket.connected',{socketId:getId()});}).on('disconnect',function(){$rootScope.$broadcast('socket.disconnected');}).on('news',function(data){$rootScope.$broadcast('socket.updates',{data:data});}).on('twitter.connected',function(){$rootScope.$broadcast('twitter.connected');}).on('twitter.reconnecting',function(data){var waitSec=data/1000;$rootScope.$broadcast('twitter.reconnecting',{waitSec:waitSec});});}
function getId(){return _socket.socket.sessionid;}}})(angular);(function(angular){'use strict';angular.module('twitStreamMock').factory('Socket',Socket);function Socket($rootScope){var _events={};var _socketId='';var service={emit:emit,connect:connect,listen:listen,on:on,getId:getId};return service;function emit(eventName,data){var emitData={};if(typeof data!=='undefined'){emitData={params:data};}
if(_events[eventName]){angular.forEach(_events[eventName],function(callback){callback(emitData);});}}
function connect(){_socketId=Math.random().toString(36).slice(2);}
function listen(){return true;}
function on(eventName,callback){if(!_events[eventName]){_events[eventName]=[];}
_events[eventName].push(callback);}
function getId(){return _socketId;}}})(angular);(function(angular){'use strict';angular.module('twitStream').factory('Storage',Storage);function Storage($rootScope,localStorageService){var _storageKey=null;var _response={};var _responseTypes={success:{message:'Added to "Favorites".',bootstrapClass:'success',type:'ok'},error:{message:'Adding to "Favorites" failed.',bootstrapClass:'danger',type:'error'},notEnoughSpace:{message:'You don\'t have enough space in the browser local storage.',bootstrapClass:'warning',type:'error'},deleteSuccess:{message:'Removed from "Favorites".',bootstrapClass:'success',type:'ok'},deleteError:{message:'Removing from "Favorites" failed.',bootstrapClass:'danger',type:'error'},deleteAllSuccess:{message:'All from "Favorites" is removed.',bootstrapClass:'success',type:'ok'},deleteAllError:{message:'Removing all from "Favorites" failed.',bootstrapClass:'danger',type:'error'}};var _storedIndexes=[];var service={isSupported:isSupported,isStored:isStored,remove:remove,removeAll:removeAll,addToFavorites:addToFavorites,getFromFavorites:getFromFavorites,getResponse:getResponse};return service;function _storeIndex(value){_storedIndexes.push(value);};function _removeIndex(value){var i=_storedIndexes.indexOf(value);if(i>-1){_storedIndexes.splice(i,1);}};function _removeAllIndexes(){_storedIndexes=[];};function _getData(){return localStorageService.get(_storageKey);};function _setData(value){return localStorageService.set(_storageKey,value);};function isSupported(){return localStorageService.isSupported;}
function isStored(tweetId){return _storedIndexes.indexOf(tweetId)!==-1;}
function remove(tweetId){var tweets;var restTweets=[];try{tweets=_getData();if(!tweets||tweets==='null'){_response=_responseTypes.deleteError;}
else{delete tweets[tweetId];angular.forEach(tweets,function(value,key){this.push(value);},restTweets);_setData(tweets);_removeIndex(tweetId);_response=_responseTypes.deleteSuccess;}}
catch(e){_response=_responseTypes.deleteError;}
$rootScope.$broadcast('favorites.removed',{status:getResponse()});}
function removeAll(){try{localStorageService.remove(_storageKey);_removeAllIndexes();_response=_responseTypes.deleteAllSuccess;}
catch(e){_response=_responseTypes.deleteAllError;}
$rootScope.$broadcast('favorites.removedAll',{status:getResponse()});}
function addToFavorites(key,tweet){var value;try{_storageKey=key;value=_getData();if(!value||value==='null'){value={};}
value[tweet.idStr]=tweet;_setData(value);_storeIndex(tweet.idStr);_response=_responseTypes.success;}
catch(e){if(e.name==='QUOTA_EXCEEDED_ERR'||e.name==='NS_ERROR_DOM_QUOTA_REACHED'){_response=_responseTypes.notEnoughSpace;}
else{console.log(e);_response=_responseTypes.error;}}
$rootScope.$broadcast('favorites.added',{status:getResponse()});}
function getFromFavorites(){var _return=[];var data=_getData();if(data){angular.forEach(data,function(value,key){this.push(value);},_return);}
return _return;}
function getResponse(){return _response;}}})(angular);(function(angular){'use strict';angular.module('twitStream').factory('StreamStatus',StreamStatus);function StreamStatus(){var _tmp={};var service={set:set,get:get};return service;function set(key,value){_tmp[key]=value;}
function get(key){return _tmp[key];}}})(angular);(function(angular){'use strict';angular.module('twitStream').factory('Tweet',Tweet);function Tweet(){var service={parse:parse};return service;function parse(tweet){return{user:{avatarUrl:tweet.user.profile_image_url,screenName:tweet.user.screen_name,name:tweet.user.name},source:tweet.source,entities:linkifyEntities(tweet),idStr:tweet.id_str,created:tweetDateString(tweet.created_at)};}
function tweetDateString(time_value){var values=time_value.split(" ");time_value=values[1]+" "+values[2]+", "+values[5]+" "+values[3];var parsed_date=Date.parse(time_value);var relative_to=(arguments.length>1)?arguments[1]:new Date();var delta=parseInt((relative_to.getTime()-parsed_date)/1000,10);delta=delta+(relative_to.getTimezoneOffset()*60);var r='';if(delta<60){r='a minute ago';}else if(delta<120){r='couple of minutes ago';}else if(delta<(45*60)){r=(parseInt(delta/60,10)).toString()+' minutes ago';}else if(delta<(90*60)){r='an hour ago';}else if(delta<(24*60*60)){r=''+(parseInt(delta/3600,10)).toString()+' hours ago';}else if(delta<(48*60*60)){r='1 day ago';}else{r=(parseInt(delta/86400,10)).toString()+' days ago';}
return r;}
function linkifyEntities(tweet){if(!(tweet.entities)){return tweet.text;}
var index_map={};if(typeof tweet.entities.urls!='undefined'){tweet.entities.urls.forEach(function(entry){index_map[entry.indices[0]]=[entry.indices[1],function(text){return'<a href="'+entry.url+'"><b>'+entry.url+'</b></a>';}];});}
if(typeof tweet.entities.hashtags!='undefined'){tweet.entities.hashtags.forEach(function(entry){index_map[entry.indices[0]]=[entry.indices[1],function(text){return'<a href="http://twitter.com/search?q='+escape("#"+entry.text)+'"&src=hash"><s>'+text.substring(0,1)+'</s><b>'+text.substring(1,text.length)+'</b></a>';}];});}
if(typeof tweet.entities.user_mentions!='undefined'){tweet.entities.user_mentions.forEach(function(entry){index_map[entry.indices[0]]=[entry.indices[1],function(text){return'<a href="http://twitter.com/'+entry.screen_name+'"><s>'+text.substring(0,1)+'</s><b>'+text.substring(1,text.length)+'</b></a>';}];});}
if(typeof tweet.entities.media!='undefined'){tweet.entities.media.forEach(function(entry){index_map[entry.indices[0]]=[entry.indices[1],function(text){return'<div class="tweet-media"><img src="'+entry.media_url+'"></div>';}];});}
var result='';var last_i=0;var i=0;for(i=0;i<tweet.text.length;++i){var ind=index_map[i];if(ind){var end=ind[0];var func=ind[1];if(i>last_i){result+=tweet.text.substring(last_i,i);}
result+=func(tweet.text.substring(i,end));i=end-1;last_i=end;}}
if(i>last_i){result+=tweet.text.substring(last_i,i);}
