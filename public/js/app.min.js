...
function directiveTemplateContents(template){var type;template=trim(template);if((type=TABLE_CONTENT_REGEXP.exec(template))){type=type[1].toLowerCase();var table=jqLite('<table>'+template+'</table>');if(/(thead|tbody|tfoot)/.test(type)){return table.children(type);}
table=table.children('tbody');if(type==='tr'){return table.children('tr');}
return table.children('tr').contents();}
return jqLite('<div>'+
template+'</div>').contents();}
...
function completeRequest(callback,status,response,headersString){timeoutId&&$browserDefer.cancel(timeoutId);jsonpDone=xhr=null;if(status===0){status=response?200:urlResolve(url).protocol=='file'?404:0;}
...
return value;};ctrl.$parsers.push(validator);ctrl.$formatters.push(validator);}}
...
var closingTimer=null;var closingTimestamp=0;var animationElementQueue=[];function animationCloseHandler(element,totalTime){var node=extractElementNode(element);element=angular.element(node);animationElementQueue.push(element);var futureTimestamp=Date.now()+(totalTime*1000);if(futureTimestamp<=closingTimestamp){return;}
...
animationCompleted();},removeClass:function(element,className,animationCompleted){return animateAfter('removeClass',element,suffixClasses(className,'-remove'),animationCompleted);}};function suffixClasses(classes,suffix){var className='';classes=angular.isArray(classes)?classes:classes.split(/\s+/);forEach(classes,function(klass,i){if(klass&&klass.length>0){className+=(i>0?' ':'')+klass+suffix;}});return className;}}]);}]);})(window,window.angular);'use strict';var App=angular.module('twitStream',['ngRoute','ngSanitize','ngAnimate']).config(function($routeProvider){$routeProvider.when('/',{templateUrl:'/views/partial/stream.html',controller:'StreamController'}).when('/storage',{templateUrl:'/views/partial/favorites.html',controller:'FavoritesController'}).otherwise({redirectTo:'/'});});App.controller('StreamController',['$location','$scope','$timeout','Socket','Tweet','StreamStatus',function($location,$scope,$timeout,Socket,Tweet,StreamStatus){$scope.socketInited=StreamStatus.socketInited||false;$scope.loading=StreamStatus.loading||false;$scope.showHint=StreamStatus.showHint||false;$scope.hasResults=StreamStatus.hasResults||false;$scope.results=StreamStatus.results||[];$scope.addedToFavorites=false;$scope.search=function(){var streamOn=StreamStatus.connected||false;$scope.channels=$scope.channels||StreamStatus.channels;if($scope.channels){$scope.loading=true;if(!$scope.socketInited||streamOn){if(!streamOn){Socket.connect();}
if(typeof $scope.$$listeners['socket.updates']=='undefined'){$scope.$on('socket.updates',function(){$scope.$apply(function(){$scope.hasResults=true;$scope.loading=false;$scope.results.unshift(Tweet.parse(Socket.response));});});}
if(typeof $scope.$$listeners['socket.updates']=='undefined'){$scope.$on('socket.connected',function(){$scope.socketInited=true;});}
if(typeof $scope.$$listeners['socket.updates']=='undefined'){$scope.$on('socket.disconnected',function(){$scope.socketInited=false;});}}
var channels=$scope.channels.split(',');Socket.emit(channels);storeStatus();}};var storeStatus=function(){StreamStatus.socketInited=$scope.socketInited;StreamStatus.loading=$scope.loading;StreamStatus.showHint=false;StreamStatus.hasResults=true;StreamStatus.channels=$scope.channels;StreamStatus.results=$scope.results;StreamStatus.connected=true;};$scope.toggleHint=function(){$scope.showHint=!$scope.showHint;};$scope.addToFavorites=function(tweet,added){if(!added){if(typeof $scope.$$listeners['favorites.added']=='undefined'){$scope.$on('favorites.added',function(){$scope.addedToFavorites=true;$scope.favoritesResponse=Tweet.getResponse();$timeout(function(){$scope.addedToFavorites=false;},1500);});}
Tweet.addToFavorite(tweet);}};}]).controller('FavoritesController',['$location','$scope',function(loc,scope){}]);'use strict';App.factory('Socket',function($rootScope){return{socket:null,response:{},emit:function(channels){this.socket.emit('addChanel',{channels:channels});},connect:function(){this.socket=io.connect('/');this.listen();},listen:function(){this.socket.on('connect',function(){$rootScope.$broadcast('socket.connected');}).on('disconnect',function(){$rootScope.$broadcast('socket.disconnected');}).on('news',function(data){this.response=data;$rootScope.$broadcast('socket.updates');}.bind(this));}};});App.factory('Tweet',function($rootScope){return{responseTypes:{success:{message:'Added to favorites!',bootstrapClass:'success'},error:{message:'Added to favorites failed..',bootstrapClass:'danger'},notEnoughSpace:{message:'You don\'t have enough space in browser local storage..',bootstrapClass:'warning'}},response:{},parse:function(tweet){function tweetDateString(time_value){var values=time_value.split(" ");time_value=values[1]+" "+values[2]+", "+values[5]+" "+values[3];var parsed_date=Date.parse(time_value);var relative_to=(arguments.length>1)?arguments[1]:new Date();var delta=parseInt((relative_to.getTime()-parsed_date)/1000,10);delta=delta+(relative_to.getTimezoneOffset()*60);var r='';if(delta<60){r='a minute ago';}else if(delta<120){r='couple of minutes ago';}else if(delta<(45*60)){r=(parseInt(delta/60,10)).toString()+' minutes ago';}else if(delta<(90*60)){r='an hour ago';}else if(delta<(24*60*60)){r=''+(parseInt(delta/3600,10)).toString()+' hours ago';}else if(delta<(48*60*60)){r='1 day ago';}else{r=(parseInt(delta/86400,10)).toString()+' days ago';}
return r;}
...
return result;}
return{user:{avatarUrl:tweet.user.profile_image_url,screenName:tweet.user.screen_name,name:tweet.user.name},source:tweet.source,entities:linkifyEntities(tweet),idStr:tweet.id_str,created:tweetDateString(tweet.created_at)};},addToFavorite:function(tweet){this.response=this.responseTypes.error;$rootScope.$broadcast('favorites.added');},getResponse:function(){return this.response;}};});App.factory('StreamStatus',function(){return{};});