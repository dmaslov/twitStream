...
animationCompleted();},removeClass:function(element,className,animationCompleted){return animateAfter('removeClass',element,suffixClasses(className,'-remove'),animationCompleted);}};function suffixClasses(classes,suffix){var className='';classes=angular.isArray(classes)?classes:classes.split(/\s+/);forEach(classes,function(klass,i){if(klass&&klass.length>0){className+=(i>0?' ':'')+klass+suffix;}});return className;}}]);}]);})(window,window.angular);'use strict';var App=angular.module('twitStream',['ngRoute','ngSanitize','ngAnimate']).config(function($routeProvider){$routeProvider.when('/',{controller:'IndexController'}).when('/storage',{templateUrl:'views/storage.html',controller:'StorageController'}).otherwise({redirectTo:'/'});});App.controller('IndexController',['$location','$scope','Socket','Tweet',function($location,$scope,Socket,Tweet){$scope.socketInited=false;$scope.loading=false;$scope.showHint=false;$scope.hasResults=false;$scope.results=[];$scope.search=function(){console.log($scope);if($scope.channels){$scope.loading=true;if(!$scope.socketInited){Socket.connect();$scope.$on('socket.updates',function(){$scope.$apply(function(){$scope.hasResults=true;$scope.loading=false;$scope.results.unshift({tweet:Tweet.parse(Socket.response)});});});$scope.$on('socket.connected',function(){$scope.socketInited=true;});$scope.$on('socket.disconnected',function(){$scope.socketInited=false;});}
var channels=$scope.channels.split(',');Socket.emit(channels);}};$scope.toggleHint=function(){$scope.showHint=!$scope.showHint;};}]).controller('StorageController',['$location','$scope',function(loc,scope){}]);'use strict';App.factory('Socket',function($rootScope){return{socket:null,response:{},emit:function(channels){this.socket.emit('addChanel',{channels:channels});},connect:function(){this.socket=io.connect('/');this.listen();},listen:function(){this.socket.on('connect',function(){$rootScope.$broadcast('socket.connected');console.log('connected');}).on('disconnect',function(){$rootScope.$broadcast('socket.disconnected');console.log('disconnected');}).on('news',function(data){this.response=data;$rootScope.$broadcast('socket.updates');}.bind(this));}};});App.factory('Tweet',function(){return{parse:function(tweet){function escapeHTML(text){return text;}
