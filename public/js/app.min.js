...
if(typeof tweet.entities.user_mentions!='undefined'){tweet.entities.user_mentions.forEach(function(entry){index_map[entry.indices[0]]=[entry.indices[1],function(text){return"<a title='"+entry.name+"' href='http://twitter.com/"+entry.screen_name+"'><s>"+text.substring(0,1)+"</s><b>"+text.substring(1,text.length)+"</b></a>";}];});}
...
return result;}
return{user:{avatarUrl:tweet.user.profile_image_url,screenName:tweet.user.screen_name,name:tweet.user.name},source:tweet.source,entities:linkifyEntities(tweet),idStr:tweet.id_str,created:tweetDateString(tweet.created_at)};},addToFavorite:function(tweet){$rootScope.$broadcast('favorites.added');}};});App.factory('StreamStatus',function(){return{};});