...
scope[key]=value;scope.$watchCollection(key,function(newVal){addToLocalStorage(key,newVal);});};return{isSupported:browserSupportsLocalStorage,getStorageType:getStorageType,set:addToLocalStorage,add:addToLocalStorage,get:getFromLocalStorage,keys:getKeysForLocalStorage,remove:removeFromLocalStorage,clearAll:clearAllFromLocalStorage,bind:bindToScope,deriveKey:deriveQualifiedKey,cookie:{set:addToCookies,add:addToCookies,get:getFromCookies,remove:removeFromCookies,clearAll:clearAllFromCookies}};}];});}).call(this);var App=angular.module('twitStream',['ngRoute','ngSanitize','ngAnimate','LocalStorageModule']).config(function($routeProvider){'use strict';$routeProvider.when('/',{templateUrl:'/partial/stream.html',controller:'StreamController'}).when('/storage',{templateUrl:'/partial/favorites.html',controller:'FavoritesController'}).otherwise({redirectTo:'/'});});var AppMock=angular.module('twitStreamMock',[]);App.controller('StreamController',['$scope','$timeout','$location','$interval','Socket','Tweet','StreamStatus','Storage',function($scope,$timeout,$location,$interval,Socket,Tweet,StreamStatus,Storage){'use strict';$scope.$location=$location;$scope.controller='StreamController';$scope.socketInited=StreamStatus.get('socketInited')||false;$scope.loading=StreamStatus.get('loading')||false;$scope.showHint=StreamStatus.get('showHint')||false;$scope.hasResults=StreamStatus.get('hasResults')||false;$scope.results=StreamStatus.get('results')||[];$scope.addedToFavorites=false;$scope.socketId=StreamStatus.get('socketId')||undefined;$scope.alert={active:false,activeFor:0};var stop;$scope.search=function(){var streamOn=StreamStatus.get('connected')||false;$scope.channels=$scope.channels||StreamStatus.get('channels');if($scope.channels){$scope.loading=true;if(!$scope.socketInited||streamOn){if(!streamOn){Socket.connect();}}
var channels=$scope.channels.split(',');Socket.emit('addChanel',channels);StreamStatus.set('channels',$scope.channels);StreamStatus.set('showHint',false);StreamStatus.set('hasResults',true);StreamStatus.set('results',$scope.results);StreamStatus.set('connected',true);}};$scope.toggleHint=function(){$scope.showHint=!$scope.showHint;};$scope.addToFavorites=function(tweet){tweet.addedTimestamp=Date.now();Storage.addToFavorites($scope.socketId,tweet);};$scope.isFavorited=function(tweetId){var added=Storage.isStored(tweetId);return added;};$scope.startAlert=function(waitSec){if(angular.isDefined(stop)){return;}
$scope.alert={active:true,activeFor:waitSec};stop=$interval(function(){if(waitSec>=1){$scope.alert.activeFor=waitSec--;}
else{$scope.alert={active:false,activeFor:1};}},1000);};$scope.stopAlert=function(){if(angular.isDefined(stop)){$interval.cancel(stop);stop=undefined;}
...
if(typeof $scope.$$listeners['socket.updates']=='undefined'){$scope.$on('socket.updates',function(event,response){$scope.$apply(function(){$scope.stopAlert();$scope.hasResults=true;$scope.loading=false;$scope.results.unshift(Tweet.parse(response.data));StreamStatus.set('loading',false);});});}
if(typeof $scope.$$listeners['socket.connected']=='undefined'){$scope.$on('socket.connected',function(event,response){$scope.socketInited=true;$scope.socketId=response.socketId;StreamStatus.set('socketInited',true);StreamStatus.set('socketId',response.socketId);});}
if(typeof $scope.$$listeners['socket.disconnected']=='undefined'){$scope.$on('socket.disconnected',function(){$scope.socketInited=false;StreamStatus.set('socketInited',false);});}
if(typeof $scope.$$listeners['twitter.connected']=='undefined'){$scope.$on('twitter.connected',function(){$scope.stopAlert();});}
if(typeof $scope.$$listeners['twitter.reconnecting']=='undefined'){$scope.$on('twitter.reconnecting',function(event,response){$scope.startAlert(response.waitSec);});}
$scope.$on('$destroy',function(){$scope.stopAlert();});}]).controller('FavoritesController',['$scope','$timeout','$location','Storage',function($scope,$timeout,$location,Storage){'use strict';$scope.$location=$location;$scope.controller='FavoritesController';$scope.tweetIndex=null;$scope.favoritesList=false;$scope.removedFromFavorites=false;$scope.q='';$scope.getAllFromFavorites=function(){var storageData=Storage.getFromFavorites();$scope.favoritesList=(angular.equals([],storageData))?false:storageData;};$scope.removeFromFavorites=function(tweet){$scope.tweetIndex=$scope.favoritesList.indexOf(tweet);Storage.remove(tweet.idStr);};$scope.removeAllFromFavorites=function(){if(!angular.equals([],$scope.favoritesList)&&$scope.favoritesList){Storage.removeAll();}};if(typeof $scope.$$listeners['favorites.removed']=='undefined'){$scope.$on('favorites.removed',function(event,response){if(response.status.type==='ok'){$scope.favoritesList.splice($scope.tweetIndex,1);}
if(angular.equals([],$scope.favoritesList)){$scope.favoritesList=false;}
$scope.removedFromFavorites=true;$scope.favoritesResponse=response.status;$timeout(function(){$scope.removedFromFavorites=false;},1500);});}
if(typeof $scope.$$listeners['favorites.removedAll']=='undefined'){$scope.$on('favorites.removedAll',function(event,response){if(response.status.type==='ok'){$scope.favoritesList=false;}
$scope.removedFromFavorites=true;$scope.favoritesResponse=response.status;$timeout(function(){$scope.removedFromFavorites=false;},1500);});}}]);App.directive('tweet',function(){return{restrict:'E',scope:true,replace:true,templateUrl:'/partial/tweet_template.html'};});App.factory('Socket',function($rootScope){'use strict';var _socket=null;return{emit:function(eventName,data){var emitData={};if(typeof data!=='undefined'){emitData={params:data};}
_socket.emit(eventName,emitData);},connect:function(){_socket=io.connect('/');this.listen();},listen:function(){_socket.on('connect',function(){$rootScope.$broadcast('socket.connected',{socketId:this.getId()});}.bind(this)).on('disconnect',function(){$rootScope.$broadcast('socket.disconnected');}).on('news',function(data){$rootScope.$broadcast('socket.updates',{data:data});}.bind(this)).on('twitter.connected',function(){$rootScope.$broadcast('twitter.connected');}).on('twitter.reconnecting',function(data){var waitSec=data/1000;$rootScope.$broadcast('twitter.reconnecting',{waitSec:waitSec});});},getId:function(){return _socket.socket.sessionid;}};});AppMock.factory('Socket',function($rootScope){'use strict';var _events={};var _socketId='';return{emit:function(eventName,data){var emitData={};if(typeof data!=='undefined'){emitData={params:data};}
if(_events[eventName]){angular.forEach(_events[eventName],function(callback){callback(emitData);});}},connect:function(){_socketId=Math.random().toString(36).slice(2);},listen:function(){return true;},on:function(eventName,callback){if(!_events[eventName]){_events[eventName]=[];}
_events[eventName].push(callback);},getId:function(){return _socketId;}};});App.factory('Tweet',function(){'use strict';return{parse:function(tweet){function tweetDateString(time_value){var values=time_value.split(" ");time_value=values[1]+" "+values[2]+", "+values[5]+" "+values[3];var parsed_date=Date.parse(time_value);var relative_to=(arguments.length>1)?arguments[1]:new Date();var delta=parseInt((relative_to.getTime()-parsed_date)/1000,10);delta=delta+(relative_to.getTimezoneOffset()*60);var r='';if(delta<60){r='a minute ago';}else if(delta<120){r='couple of minutes ago';}else if(delta<(45*60)){r=(parseInt(delta/60,10)).toString()+' minutes ago';}else if(delta<(90*60)){r='an hour ago';}else if(delta<(24*60*60)){r=''+(parseInt(delta/3600,10)).toString()+' hours ago';}else if(delta<(48*60*60)){r='1 day ago';}else{r=(parseInt(delta/86400,10)).toString()+' days ago';}
return r;}
...
return result;}
return{user:{avatarUrl:tweet.user.profile_image_url,screenName:tweet.user.screen_name,name:tweet.user.name},source:tweet.source,entities:linkifyEntities(tweet),idStr:tweet.id_str,created:tweetDateString(tweet.created_at)};}};});App.factory('StreamStatus',function(){'use strict';var _tmp={};return{set:function(key,value){_tmp[key]=value;},get:function(key){return _tmp[key];}};});App.factory('Storage',function($rootScope,localStorageService){'use strict';var _storageKey=null;var _response={};var _responseTypes={success:{message:'Added to "Favorites".',bootstrapClass:'success',type:'ok'},error:{message:'Adding to "Favorites" failed.',bootstrapClass:'danger',type:'error'},notEnoughSpace:{message:'You don\'t have enough space in the browser local storage.',bootstrapClass:'warning',type:'error'},deleteSuccess:{message:'Removed from "Favorites".',bootstrapClass:'success',type:'ok'},deleteError:{message:'Removing from "Favorites" failed.',bootstrapClass:'danger',type:'error'},deleteAllSuccess:{message:'All from "Favorites" is removed.',bootstrapClass:'success',type:'ok'},deleteAllError:{message:'Removing all from "Favorites" failed.',bootstrapClass:'danger',type:'error'}};var _storedIndexes=[];var _storeIndex=function(value){_storedIndexes.push(value);};var _removeIndex=function(value){var i=_storedIndexes.indexOf(value);if(i>-1){_storedIndexes.splice(i,1);}};var _removeAllIndexes=function(){_storedIndexes=[];};var _getData=function(){return localStorageService.get(_storageKey);};var _setData=function(value){return localStorageService.set(_storageKey,value);};return{isSupported:function(){return localStorageService.isSupported;},isStored:function(tweetId){return _storedIndexes.indexOf(tweetId)!==-1;},remove:function(tweetId){var tweets;var restTweets=[];try{tweets=_getData();if(!tweets||tweets==='null'){_response=_responseTypes.deleteError;}
...
$rootScope.$broadcast('favorites.removed',{status:this.getResponse()});},removeAll:function(){try{localStorageService.remove(_storageKey);_removeAllIndexes();_response=_responseTypes.deleteAllSuccess;}
catch(e){_response=_responseTypes.deleteAllError;}
$rootScope.$broadcast('favorites.removedAll',{status:this.getResponse()});},addToFavorites:function(key,tweet){var value;try{_storageKey=key;value=_getData();if(!value||value==='null'){value={};}
...
else{_response=_responseTypes.error;}}
$rootScope.$broadcast('favorites.added',{status:this.getResponse()});},getFromFavorites:function(){var _return=[];var data=_getData();if(data){angular.forEach(data,function(value,key){this.push(value);},_return);}
return _return;},getResponse:function(){return _response;}};});