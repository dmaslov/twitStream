...
animationCompleted();},removeClass:function(element,className,animationCompleted){return animateAfter('removeClass',element,suffixClasses(className,'-remove'),animationCompleted);}};function suffixClasses(classes,suffix){var className='';classes=angular.isArray(classes)?classes:classes.split(/\s+/);forEach(classes,function(klass,i){if(klass&&klass.length>0){className+=(i>0?' ':'')+klass+suffix;}});return className;}}]);}]);})(window,window.angular);'use strict';var App=angular.module('twitStream',['ngRoute','ngSanitize','ngAnimate']).config(function($routeProvider){$routeProvider.when('/',{templateUrl:'/views/partial/stream.html',controller:'StreamController'}).when('/storage',{templateUrl:'/views/partial/favorites.html',controller:'FavoritesController'}).otherwise({redirectTo:'/'});});App.controller('StreamController',['$location','$scope','Socket','Tweet','StreamStatus',function($location,$scope,Socket,Tweet,StreamStatus){$scope.socketInited=StreamStatus.socketInited||false;$scope.loading=StreamStatus.loading||false;$scope.showHint=StreamStatus.showHint||false;$scope.hasResults=StreamStatus.hasResults||false;$scope.results=StreamStatus.results||[];$scope.search=function(){var streamOn=StreamStatus.connected||false;$scope.channels=$scope.channels||StreamStatus.channels;if($scope.channels){$scope.loading=true;if(!$scope.socketInited||streamOn){if(!streamOn){Socket.connect();}
if(typeof $scope.$$listeners['socket.updates']=='undefined'){$scope.$on('socket.updates',function(){$scope.$apply(function(){$scope.hasResults=true;$scope.loading=false;$scope.results.unshift({tweet:Tweet.parse(Socket.response)});});});}
...
return r;}
function linkifyEntities(tweet){if(!(tweet.entities)){return escapeHTML(tweet.text);}
var index_map={};if(typeof tweet.entities.urls!='undefined'){tweet.entities.urls.forEach(function(entry){index_map[entry.indices[0]]=[entry.indices[1],function(text){return"<a href='"+escapeHTML(entry.url)+"'><b>"+escapeHTML(entry.url)+"</b></a>";}];});}
if(typeof tweet.entities.urls!='undefined'){tweet.entities.hashtags.forEach(function(entry){index_map[entry.indices[0]]=[entry.indices[1],function(text){var escapedString=escapeHTML(text);return"<a href='http://twitter.com/search?q="+escape("#"+entry.text)+"&src=hash'><s>"+escapedString.substring(0,1)+"</s><b>"+escapedString.substring(1,escapedString.length)+"</b></a>";}];});}
if(typeof tweet.entities.urls!='undefined'){tweet.entities.user_mentions.forEach(function(entry){index_map[entry.indices[0]]=[entry.indices[1],function(text){var escapedString=escapeHTML(text);return"<a title='"+escapeHTML(entry.name)+"' href='http://twitter.com/"+escapeHTML(entry.screen_name)+"'><s>"+escapedString.substring(0,1)+"</s><b>"+escapedString.substring(1,escapedString.length)+"</b></a>";}];});}
...
var result='';var last_i=0;var i=0;for(i=0;i<tweet.text.length;++i){var ind=index_map[i];if(ind){var end=ind[0];var func=ind[1];if(i>last_i){result+=escapeHTML(tweet.text.substring(last_i,i));}
result+=func(tweet.text.substring(i,end));i=end-1;last_i=end;}}
if(i>last_i){result+=tweet.text.substring(last_i,i);}
return result;}
var _tweet='<div class="container">';_tweet+='<div class="row">';_tweet+='<div class="col-md-12">';_tweet+='<div class="avatar">';_tweet+='<img class="img-rounded" src="'+tweet.user.profile_image_url+'">';_tweet+='</div>';_tweet+='<div class="tweet-info">';_tweet+='<a target="_blank" href="http://twitter.com/'+tweet.user.screen_name+'"><strong>'+tweet.user.name+'</strong> <span class="username"><s>@</s><b>'+tweet.user.screen_name+'</b></span></a>';_tweet+='<small class="time"><a href="http://twitter.com/'+tweet.user.screen_name+'/status/'+tweet.id_str+'" target="_blank">'+tweetDateString(tweet.created_at)+'</a> via '+tweet.source+'</small>';_tweet+='<div class="tweet-text">'+linkifyEntities(tweet)+'</div>';_tweet+='</div>';_tweet+='</div>';_tweet+='</div>';return _tweet;}};});App.factory('StreamStatus',function(){return{};});