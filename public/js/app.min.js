...
var channels=$scope.channels.split(',');Socket.emit(channels);StreamStatus.set('channels',$scope.channels);StreamStatus.set('showHint',false);StreamStatus.set('hasResults',true);StreamStatus.set('results',$scope.results);StreamStatus.set('connected',true);}};$scope.toggleHint=function(){$scope.showHint=!$scope.showHint;};$scope.addToFavorites=function(tweet,added){if(!added){Storage.addToFavorites($scope.socketId,tweet);}};if(typeof $scope.$$listeners['favorites.added']=='undefined'){$scope.$on('favorites.added',function(event,response){$scope.addedToFavorites=true;$scope.favoritesResponse=response.status;$timeout(function(){$scope.addedToFavorites=false;},1500);});}
...
return result;}
return{user:{avatarUrl:tweet.user.profile_image_url,screenName:tweet.user.screen_name,name:tweet.user.name},source:tweet.source,entities:linkifyEntities(tweet),idStr:tweet.id_str,created:tweetDateString(tweet.created_at)};}};});App.factory('StreamStatus',function(){'use strict';var tmp={};return{set:function(key,value){tmp[key]=value;},get:function(key){return tmp[key];}};});App.factory('Storage',function($rootScope,localStorageService){'use strict';var storageKey=null;var response={};var responseTypes={success:{message:'Added to favorites!',bootstrapClass:'success',type:'ok'},error:{message:'Adding to favorites failed..',bootstrapClass:'danger',type:'error'},notEnoughSpace:{message:'You don\'t have enough space in browser local storage..',bootstrapClass:'warning',type:'error'},deleteSuccess:{message:'Removed from favorites!',bootstrapClass:'success',type:'ok'},deleteError:{message:'Removing from favorites failed..',bootstrapClass:'danger',type:'error'},deleteAllSuccess:{message:'Removed all from favorites!',bootstrapClass:'success',type:'ok'},deleteAllError:{message:'Removing all from favorites failed..',bootstrapClass:'danger',type:'error'}};return{isSupported:function(){return localStorageService.isSupported;},set:function(value){return localStorageService.set(storageKey,value);},get:function(){return localStorageService.get(storageKey);},remove:function(tweetId){var tweets;var restTweets=[];try{tweets=this.get();if(!tweets||tweets==='null'){response=responseTypes.deleteError;}
else{delete tweets[tweetId];angular.forEach(tweets,function(value,key){this.push(value);},restTweets);this.set(tweets);response=responseTypes.deleteSuccess;}}
catch(e){response=responseTypes.deleteError;}
$rootScope.$broadcast('favorites.removed',{status:this.getResponse()});},removeAll:function(){try{localStorageService.remove(storageKey);response=responseTypes.deleteAllSuccess;}
catch(e){response=responseTypes.deleteAllError;}
$rootScope.$broadcast('favorites.removedAll',{status:this.getResponse()});},addToFavorites:function(key,tweet){var value;try{storageKey=key;value=this.get(key);if(!value||value==='null'){value={};}
value[tweet.idStr]=tweet;this.set(value);response=responseTypes.success;}
catch(e){if(e.name==='QUOTA_EXCEEDED_ERR'||e.name==='NS_ERROR_DOM_QUOTA_REACHED'){response=responseTypes.notEnoughSpace;}
else{response=responseTypes.error;}}
$rootScope.$broadcast('favorites.added',{status:this.getResponse()});},getFromFavorites:function(){var _return=[];var data=this.get();if(data){angular.forEach(data,function(value,key){this.push(value);},_return);}
return _return;},getResponse:function(){return response;}};});