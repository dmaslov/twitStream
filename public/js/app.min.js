...
scope[key]=value;scope.$watchCollection(key,function(newVal){addToLocalStorage(key,newVal);});};return{isSupported:browserSupportsLocalStorage,getStorageType:getStorageType,set:addToLocalStorage,add:addToLocalStorage,get:getFromLocalStorage,keys:getKeysForLocalStorage,remove:removeFromLocalStorage,clearAll:clearAllFromLocalStorage,bind:bindToScope,cookie:{set:addToCookies,add:addToCookies,get:getFromCookies,remove:removeFromCookies,clearAll:clearAllFromCookies}};}];});}).call(this);var App=angular.module('twitStream',['ngRoute','ngSanitize','ngAnimate','LocalStorageModule']).config(function($routeProvider){'use strict';$routeProvider.when('/',{templateUrl:'/partial/stream.html',controller:'StreamController'}).when('/storage',{templateUrl:'/partial/favorites.html',controller:'FavoritesController'}).otherwise({redirectTo:'/'});});App.controller('StreamController',['$scope','$location','$timeout','Socket','Tweet','StreamStatus','Storage',function($scope,$location,$timeout,Socket,Tweet,StreamStatus,Storage){'use strict';$scope.$location=$location;$scope.socketInited=StreamStatus.get('socketInited')||false;$scope.loading=StreamStatus.get('loading')||false;$scope.showHint=StreamStatus.get('showHint')||false;$scope.hasResults=StreamStatus.get('hasResults')||false;$scope.results=StreamStatus.get('results')||[];$scope.addedToFavorites=false;$scope.socketId=StreamStatus.get('socketId')||undefined;$scope.search=function(){var streamOn=StreamStatus.get('connected')||false;$scope.channels=$scope.channels||StreamStatus.get('channels');if($scope.channels){$scope.loading=true;if(!$scope.socketInited||streamOn){if(!streamOn){Socket.connect();}}
var channels=$scope.channels.split(',');Socket.emit(channels);StreamStatus.set('channels',$scope.channels);StreamStatus.set('showHint',false);StreamStatus.set('hasResults',true);StreamStatus.set('results',$scope.results);StreamStatus.set('connected',true);}};$scope.toggleHint=function(){$scope.showHint=!$scope.showHint;};$scope.addToFavorites=function(tweet){tweet.addedTimestamp=Date.now();Storage.addToFavorites($scope.socketId,tweet);};$scope.isFavorited=function(tweetId){var added=Storage.isStored(tweetId);return added;};if(typeof $scope.$$listeners['favorites.added']=='undefined'){$scope.$on('favorites.added',function(event,response){$scope.addedToFavorites=true;$scope.favoritesResponse=response.status;$timeout(function(){$scope.addedToFavorites=false;},1500);});}
if(typeof $scope.$$listeners['socket.updates']=='undefined'){$scope.$on('socket.updates',function(event,response){$scope.$apply(function(){$scope.hasResults=true;$scope.loading=false;$scope.results.unshift(Tweet.parse(response.data));StreamStatus.set('loading',false);});});}
